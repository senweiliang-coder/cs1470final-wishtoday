...
def train(epoch,train_loader,model, best_acc=0.0):
    ...
    save_results('log/train_log/'  + args.dataset +  "_"+str(args.Condition_G_Variance)+  "_" +args.name, epoch, loss_meter.avg, aver_acc, precision, recall, f1, auc,specificity)
...
def val(current_epoch, val_loader, model, best_acc):
    ...
    if aver_acc > best_acc:
        ...
        file_name = os.path.join(save_dir, f"{args.model_name}_{args.dataset}_{args.folder}_best_epoch_{current_epoch}__{aver_acc}.pth")
...
def test(current_epoch, test_loader, model,checkpoint):
    ...
    recall = recall_score(all_targets, all_predictions, average='weighted')
    f1 = f1_score(all_targets, all_predictions, average='weighted')
    auc, _ = compute_auc_and_specificity(all_targets, all_predictions, all_probabilities)

    print('====> acc: {:.4f}'.format(aver_acc))
    print(f'Precision: {precision:.4f} \tRecall: {recall:.4f} \tF1 Score: {f1:.4f} \tAUC: {auc:.4f}')


    return loss_meter.avg, aver_acc
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="fairvision_dr", type=str, help="fairvision_dr/fairvision_glaucoma/fairvision_amd")
    ...
    parser.add_argument("--fundus_encoder", default="convnext", type=str, help="convnext/swin")
    parser.add_argument("--oct_encoder", default="resnet3d", type=str, help="resnet3d/unetr")
    parser.add_argument("--data_root", default="/home/liang/datasets/FairVision", type=str, help="root directory of the extracted FairVision dataset")
    ...
    if args.dataset in {"fairvision_dr", "fairvision_glaucoma", "fairvision_amd"}:
        args.modalties_name = ["FUN", "OCT"]
        args.dims = [[(96, 96, 96)], [(384, 384)]]
        args.modalties = len(args.dims)
        disease_name = {
            "fairvision_dr": "DR",
            "fairvision_glaucoma": "Glaucoma",
            "fairvision_amd": "AMD",
        }[args.dataset]
        args.num_classes = 4 if disease_name == "AMD" else 2
        ...
        train_dataset = FairVisionDataset(
            ...
            noise_std=args.Condition_G_Variance,
            seed=args.seed_idx,
        )
        val_dataset = FairVisionDataset(
            dataset_root=args.data_root,
            disease=disease_name,
            mode="val",
            model_base=args.model_base,
        ...
        val_loader = DataLoader(val_dataset, batch_size=max(1, args.batch_size // 2), drop_last=False, num_workers=args.num_workers)
        test_loader = DataLoader(test_dataset, batch_size=max(1, args.batch_size // 2), drop_last=False, num_workers=args.num_workers)
    elif args.dataset =="dr2":
