from Models.fundus_convnext_network import build_model as convnext_build_model
from Models.fundus_swin_network import build_model as swin_build_model
from Models.oct_resnet3d_network import build_model as oct_resnet_build_model
...
def build_fundus_encoder(args):
    encoder_name = getattr(args, "fundus_encoder", "convnext").lower()
    if encoder_name == "convnext":
        return convnext_build_model()
    if encoder_name == "swin":
        return swin_build_model()
    raise ValueError(f"Unsupported 2D fundus encoder: {encoder_name}")


def build_oct_encoder(args, num_classes):
    encoder_name = getattr(args, "oct_encoder", "resnet3d").lower()
    if encoder_name == "resnet3d":
        return oct_resnet_build_model(num_classes=num_classes)
    if encoder_name == "unetr":
        return UNETR_base_3DNet(num_classes=num_classes)
    raise ValueError(f"Unsupported 3D OCT encoder: {encoder_name}")
...
class FDE(nn.Module):
    def __init__(self, args,common_ratio = 0.5):
        ...
        self.guided_features_projector1 = nn.Linear(256,int(2048 * common_ratio) )
        self.guided_features_projector2 = nn.Linear(256,int(2048 * common_ratio) )
...
class MedFusion(nn.Module):
    def __init__(self, classes, modalties, classifiers_dims, args):
        ...
        self.transformer_2DNet = build_fundus_encoder(args)

        # ---- 3D Transformer Backbone ----
        self.transformer_3DNet = build_oct_encoder(args, self.classes)

        self.fc_fundus = nn.Sequential(nn.ReLU(), nn.Linear(self.num_classes * self.dim_general, 1024), nn.ReLU())
        ...
        self.KFR_fundus = KFR(self.fundus_embedding_dim, num_classes=self.num_classes,
                              topk=self.topk_fundus, sample_num=self.sample_num, seed=self.seed, batch_size=args.batch_size)

        self.KFR_oct = KFR(self.oct_embedding_dim, num_classes=self.num_classes,
        ...
        self.FDE = FDE(args,common_ratio=0.5)
    ...
    def forward(self, X, y, epoch):
        ...
        # mu_topk_fundus, sigma_topk_fundus, proxy_loss_fundus, z_topk_fundus, v_fundus = self.KFR_fundus(fundus_out,
        #                                                                                                 y=y)  # mu_top torch.Size([8, 2, 256])
        # mu_topk_oct, sigma_topk_oct, proxy_loss_oct, z_topk_oct, v_oct = self.KFR_oct(oct_out, y=y)
        if not self.training:
            mu_topk_fundus, sigma_topk_fundus, proxy_loss_fundus, z_topk_fundus, entropy_loss = self.KFR_fundus(x, y=y)  # mu_top torch.Size([8, 2, 256])
            mu_topk_oct, sigma_topk_oct, proxy_loss_oct, z_topk_oct, entropy_loss  = self.KFR_oct(x1, y=y)
        else:
            mu_topk_fundus, sigma_topk_fundus, proxy_loss_fundus, z_topk_fundus = self.KFR_fundus(x, y=y)  # mu_top torch.Size([8, 2, 256])
            mu_topk_oct, sigma_topk_oct, proxy_loss_oct, z_topk_oct = self.KFR_oct(x1, y=y)
        ...
        combine_features, loss_FDE = self.FDE(x, x1, global_fusion,fundus_guided, oct_guided)
        ...
        if not self.training:
            loss = self.compute_loss_test(loss1, IB_loss_proxy, proxy_loss_fundus, proxy_loss_oct, loss_FDE, entropy_loss)
        else:
            loss = self.compute_loss_train(loss1, IB_loss_proxy, proxy_loss_fundus, proxy_loss_oct, loss_FDE)
